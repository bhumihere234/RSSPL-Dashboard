  ...
  return (
    <aside className="w-full md:w-80 bg-[#0e0f12] border-l border-neutral-800 text-neutral-200 flex flex-col">
      <div className="p-4 border-b border-neutral-800">
        ...
        <div className="mt-2 text-4xl md:text-5xl font-extrabold tracking-tight text-white">{formatTime(now)}</div>
      ...
      <ScrollArea className="flex-1">
        <ul className="p-3 space-y-2">
          ...
          {state.notifications.length === 0 && <li className="text-xs text-neutral-500 px-1">No notifications</li>}
        </ul>
      </ScrollArea>
      ...
      <div className="px-4 py-3 border-t border-neutral-800">
        ...
        <ScrollArea className="max-h-48">
          <ul className="space-y-2">
            ...
            {state.messages.length === 0 && <li className="text-xs text-neutral-500">No messages</li>}
          </ul>
        </ScrollArea>
      </div>
    </aside>
...
function MessageRow({ id, text, checkedInit, at }: { id: string; text: string; checkedInit: boolean; at: number }) {
  const { state, resolveMessage } = useInventory() as any
  const [checked, setChecked] = useState(checkedInit)
  useEffect(() => {
    setChecked(checkedInit)
  }, [checkedInit, state.messages.length])
  return (
    <li className={cn("bg-neutral-900/60 border border-neutral-800 rounded-md p-3 flex items-start gap-3")}>
      <button
      ...
      <div className="flex-1">
        ...
        <div className="text-[11px] text-neutral-500">{new Date(at).toLocaleString()}</div>
      </div>
    </li>
  )
}
