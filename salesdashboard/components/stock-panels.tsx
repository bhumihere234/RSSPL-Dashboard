...
export default function StockPanels() {
  ...
  // catalog options (use union lists from state)
  const itemOptions = inv.state.catalogItems;
  const typeOptionsFor = (it?: string) =>
    it ? inv.state.catalogTypesByItem[it] ?? [] : [];

  // Selection
  const [item, setItem] = React.useState<string | undefined>(itemOptions[0]);
  const [type, setType] = React.useState<string>("");

  // keep selection consistent when catalogs change
  React.useEffect(() => {
    if (!item || !itemOptions.includes(item)) {
      const first = itemOptions[0];
      setItem(first);
      setType(first ? (typeOptionsFor(first)[0] ?? "") : "");
    } else {
      // ensure type exists for current item
      const types = typeOptionsFor(item);
      if (!types.includes(type)) {
        setType(types[0] ?? "");
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [itemOptions.join(","), JSON.stringify(inv.state.catalogTypesByItem)]);
  ...
  const handleStockOut = () => {
    if (item && type && qout && Number(qout) > 0) {
      inv.stockOut(item, type, Number(qout));
      setQout("");
    }
  };

  // remove item/type UX: also adjust current selection if we removed the selected one
  const handleRemoveItem = (name: string) => {
    inv.removeItem(name);
    if (item === name) {
      const nextItems = itemOptions.filter((i) => i !== name);
      const nextItem = nextItems[0];
      setItem(nextItem);
      setType(nextItem ? (typeOptionsFor(nextItem)[0] ?? "") : "");
    }
  };
  const handleRemoveType = (name: string) => {
    ...
    if (type === name) {
      const nextTypes = typeOptionsFor(item).filter((t) => t !== name);
      setType(nextTypes[0] ?? "");
    }
  };
  ...
  const handleDownloadExcel = () => {
    const rows = reportResults.map((r) => ({
      "Selected Date": new Date(r.at).toLocaleDateString(),
      ...
      <Card className="bg-neutral-900/60 border-neutral-800 md:col-span-2">
        ...
        <CardContent className="space-y-3">
          <ItemDropdown
            ...
            onSelect={(v) => {
              setItem(v);
              const first = typeOptionsFor(v)[0] ?? "";
              setType(first);
            }}
        ...
        <CardContent className="space-y-3">
          ...
          <div className="flex gap-2 items-center">
            <Input
              ...
              placeholder="Enter the quantity..."
              className="flex-1 bg-neutral-900 border-neutral-800 text-neutral-100 placeholder:text-neutral-500 placeholder:opacity-60"
            />
      ...
      <div className="md:col-span-3 mt-4 bg-neutral-900/60 border border-neutral-800 rounded-md p-6">
        ...
        {(() => {
          ...
          return results.length > 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-neutral-400">
                    <th className="px-2 py-1 text-left">Selected Date</th>
            ...
            <p className="text-neutral-500">No records found for the selected filters.</p>
          );
        })()}
        <Button
